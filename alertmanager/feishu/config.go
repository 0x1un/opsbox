...
type Config struct {
	...
	OSS       map[string]OSSConfig      `yaml:"oss"`
...
func (g *GrafanaConfig) GetURL() string {
	return g.URL
}

func (sc *SafeConfig) GetGrafana() *GrafanaConfig {
	sc.Lock()
	defer sc.Unlock()
	return &sc.C.Grafana
}

func (sc *SafeConfig) GetAliyunOSS() *OSSConfig {
	sc.Lock()
	defer sc.Unlock()
	oss, ok := sc.C.OSS["aliyun"]
	if ok {
		return &oss
	}
	return nil
